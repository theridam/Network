...
function [ G,E,V,road,junction,grid ] = num_sol( nodelist1,nodelist2,L,a,b,N,T,eps,mu,rMax,r0,p0,CFL,alpha,c,f,sigma,fin,s,d,sB,dB,sB2,dB2,d1,method,outflowType,cType )

    % Compute grid variables
    [ grid ] = grid_variables( L,N,T,CFL );
    
    % Generate the graph of the network
    [ G,E,V,indeg,outdeg ] = generate_graph( nodelist1,nodelist2,L );
    ...
    % Change initial data in special cases
    % [road] = initial_data_shock(road,grid);
    % [road] = initial_data_rarefaction(road,grid);
    
    for tn = 1:grid.NT-1
        
        for v = 1:V
            % Inflow and Outflow
            [ road,junction ] = inflow_outflow( v,tn,grid.tau,road,junction,f,sigma,fin,s,d,sB,dB,d1,sB2,dB2,eps,outflowType,cType );
    ...
    % Inflow and Outflow at final time
    tn = grid.NT;
    for v=1:V
        [ road,junction ] = inflow_outflow( v,tn,grid.tau,road,junction,f,sigma,fin,s,d,sB,dB,d1,sB2,dB2,eps,outflowType,cType );
    end
