...
% Analytical solution
xexact = @(t) (-2+0.5*t)*(t<=(8/3)) + (-(2/3)+0.25*(t-(8/3)))*(t>(8/3));
...
% plot(xe,t,-0.25*t,t)
% axis([-2 2 0 12])
% legend('car trajectory','shock front')
% xlabel('x')
% ylabel('t')
...
% data_time   = [n', time];
% 
% csvwrite_with_headers('data_xct_shock.csv',data_xct,header);
% csvwrite_with_headers('data_xdiff_shock.csv',data_xdiff,header);
% csvwrite_with_headers('data_errct_shock.csv',data_errct,header);
% csvwrite_with_headers('data_errmax_shock.csv',data_errmax,header);
% csvwrite_with_headers('data_time_shock.csv',data_time,header);
...
subplot(1,2,1)
plot(N,x_ct(:,1),N,x_ct(:,2),N,x_ct(:,3),N,x_ct(:,4),N,x_ct(:,5),N,(5/3)*ones(length(N),1))
xlabel('N')
ylabel('x(12)')
